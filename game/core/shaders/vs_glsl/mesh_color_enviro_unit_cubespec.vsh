...
void main()
{
    ...
    v_vReflect.xyz    = reflect(vPositionOffset, vWVNormal);
    
    float fFresnelBias = -1.5;
    float fFresnelScale = 4.0;
    float fFresnelPower = 1.2;
    v_vReflect.w = clamp(fFresnelBias + fFresnelScale * pow(clamp(1.0 + dot(normalize(vPositionOffset), normalize(vWVNormal)), 0.0, 1.0), fFresnelPower), 0.0, 1.0);
