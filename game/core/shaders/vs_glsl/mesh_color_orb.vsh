...
void main()
{
    ...
    vTangent = normalize((vec4(a_vTangent, 0.0) * mBlend).xyz);
#else
    ...
    gl_TexCoord[0].xy = gl_MultiTexCoord0.xy + vTex0a;
    gl_TexCoord[0].zw = gl_MultiTexCoord0.xy + vTex0b;
    gl_TexCoord[1].xy = gl_MultiTexCoord1.xy + vTex1a;
    gl_TexCoord[1].zw = gl_MultiTexCoord1.xy + vTex1b;
    v_vColor          = vColor;
    ...
    v_vRefract.xyz    = vWVNormal;
    
    float fFresnelBias = 0.2;
    float fFresnelScale = 0.8;
    float fFresnelPower = 7.0;
    v_vRefract.w = 1.0 - clamp(fFresnelBias + fFresnelScale * pow(1.0 + dot(-vCamDirection, normalize(vWVNormal)), fFresnelPower), 0.0, 1.0);
#endif
    
#if (LIGHTING_QUALITY == 0 || LIGHTING_QUALITY == 1 || FALLOFF_QUALITY == 0)
    v_vPositionView  = vPositionView;
#else
    v_vPositionScreen.z = vPositionView.z;
#endif
